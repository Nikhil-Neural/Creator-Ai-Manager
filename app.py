...
if current_os_mode == "✍️ AI Script Generator":
    ...
    with tab2:
        ...
        if "script_data" in st.session_state and st.session_state["script_data"]:
            ...
            with c2: st.download_button("📥 Word Doc (.docx)", create_word_doc(str(st.session_state["script_data"]), platform, st.session_state.get("niche_data", "File")), file_name=f"{safe_name}.docx", use_container_width=True)
